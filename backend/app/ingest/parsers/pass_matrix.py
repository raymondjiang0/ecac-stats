...
Text layout (from extract_text()):
  ...
  Data rows (groups of 3 lines per player):
    - jersey number (1-2 digit string)
    - space-separated cell values ("—" or integer), ending with TOTAL cell
    - last name
  Final line: "TOTAL ..."
"""
import re
...
def _parse_pass_cell(cell: str) -> Optional[int]:
    """Parse a pass-matrix cell into an integer count.

    Returns the integer value for digit strings.
    Returns None for bare '—' or unparseable input (treat as 0, omit).
    """
    s = cell.strip()
    if not s or s == "—" or s == "-":
        return None
...
def parse_pass_matrix(pdf: pdfplumber.PDF, our_team: str) -> list[dict]:
    """Return non-zero from→to pass pairs from the PASSES DISTRIBUTION page.
    ...
    Returns:
        list of dicts, one per non-zero from→to pair:
            from_jersey : str
        ...
        Pairs where count is 0 (cell is '—') are omitted.
        Self-pass cells (from_jersey == to_jersey) are omitted.
    """
...
def _extract_pass_pairs(text: str) -> list[dict]:
    ...
    Uses _parse_pass_cell to parse each integer cell.
    Skips self-pass cells and cells that parse to None (i.e. bare '—').
    """
    ...
    # Build output: for each row player × each col player, emit if non-zero
    results: list[dict] = []
    for row_idx, (from_jersey, from_name) in enumerate(players):
        for col_idx, cell in enumerate(cell_rows[row_idx]):
            # Skip self-pass cells — a player can't pass to themselves
            if col_jerseys[col_idx] == from_jersey:
                continue
            count = _parse_pass_cell(cell)
            # Skip zero/empty cells
